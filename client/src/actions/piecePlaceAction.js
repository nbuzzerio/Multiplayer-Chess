...
export default function setPlacedPiece(coord, state, tileProps, name) {
  ...
  return function (dispatch) {
    ...
    held.style.backgroundImage =
      "url(https://multiplayer-chess.s3.amazonaws.com/Empty.png)";
    ...
    if (state.turn === "white") {
      ...
      fetch(`${window.location.href}updateGame`, {
        method: "POST",
        headers: {
        ...
        .then(() => {
          if (turnContinue) {
            ...
            message = `${move.atkPiece} ${translateCoords(
              move.atkFrom
            )} to ${translateCoords(move.defFrom)}.`;
          } else {
            message = `${move.atkPiece} ${translateCoords(
              move.atkFrom
            )} to ${translateCoords(
              move.defFrom
            )}. White has captured Black's ${move.defPiece}!`;
          }
        ...
        .then(() => {
          if (turnContinue) {
            ...
            message = `${move.atkPiece} ${translateCoords(
              ...
              move.defFrom
            )}. Black has captured White's ${move.defPiece}!`;
          }
